...
void demo5(Data& d)
{
	...
	if (isKeyPressed(SDL_SCANCODE_LEFT))   if (d.x > 0) d.x--;    // Move the previous rectangle
	if (isKeyPressed(SDL_SCANCODE_RIGHT))  if (d.x < 499) d.x++;
	if (isKeyPressed(SDL_SCANCODE_DOWN))   if (d.y > 0) d.y--;
	if (isKeyPressed(SDL_SCANCODE_UP))     if (d.y < 499) d.y++;
...
void demo7()
{
    ...
    for(i=0;i<12;++i)
    {
        ...
        b = -(-0.5f*M_PI + 2.f*M_PI*elapsedTime());
        color(255,45,23);
...
void menu(Data& d)
{
    ...
    for(i=0;i<d.n;++i)
    {
        print(WIN_DIM_X-100+10, caseToPixel(d,i+1)+20, "Démo");
